...
export default function NgeBldgHPage(){
  ...
  const fetchParkingLotData = async () => {
    try {
      ...
      const filteredSlots = temp_slots.filter((slot) => slot && slot.parkingLotID === 2);
  ...
  const fetchParkingSlotData = () => {
    ...
    const filteredSlots = temp_slots.filter(slot => slot.parkingLot && slot.parkingLot.parkingLotID === 2);
