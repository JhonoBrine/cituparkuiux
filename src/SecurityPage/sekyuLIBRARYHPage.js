...
export default function sekyuLibraryHPage(){
  ...
  const fetchParkingLotData = async () => {
    try {
      ...
      const filteredSlots = temp_slots.filter((slot) => slot && slot.parkingLotID === 5);
  ...
  const fetchParkingSlotData = () => {
    ...
    const filteredSlots = temp_slots.filter(slot => slot.parkingLot && slot.parkingLot.parkingLotID === 5);
