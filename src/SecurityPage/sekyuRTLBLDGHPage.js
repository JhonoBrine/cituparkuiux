...
export default function sekyuRtlBldgHPage(){
  ...
  const fetchParkingLotData = async () => {
    try {
      ...
      const filteredSlots = temp_slots.filter((slot) => slot && slot.parkingLotID === 1);
  ...
  const fetchParkingSlotData = () => {
    ...
    const filteredSlots = temp_slots.filter(slot => slot.parkingLot && slot.parkingLot.parkingLotID === 1);
